...
import { fetchProductById } from '../api/items'
  ...
  useEffect(() => {
    ...
    async function loadProduct() {
      try {
        ...
        if (!ignore) setError(err.message || 'Unable to load product')
      } finally {
    ...
    <div>
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm lg:grid lg:grid-cols-2 lg:gap-0">
        ...
        <div className="flex flex-col justify-center p-6 md:p-10">
          <p className="text-sm font-medium text-slate-500">{product.year}</p>
          ...
          <dl className="mt-8 grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <dd className="font-medium text-slate-800">{product.storage}</dd>
            </div>
            <div>
              <dt className="text-slate-500">Chip</dt>
              <dd className="font-medium text-slate-800">{product.chip}</dd>
            </div>
            <div>
              <dt className="text-slate-500">Display</dt>
              <dd className="font-medium text-slate-800">{product.display}</dd>
            </div>
            <div>
          ...
          <p className="mt-8 text-slate-600 leading-relaxed">{product.description}</p>
